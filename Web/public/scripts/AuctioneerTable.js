class AuctioneerTable {
  constructor(user) {
    this.user = user;
    this.id = user + "_view";
    this.eventClass = "org.acme.product.auction.SummitCar";
  }
  loadData(data) {
    //console.log(data);
    ...
    for(var i = 0; i < data.length; i++) {
      ...
      var rowData = "";
      if(this.user === "car") {
        rowData = "<tr ><td width='10%'>" + eventData["carid"] + "</td><td width='10%'>" + eventData["brand"] + "</td><td width='10%'>" + eventData["model"] + "</td><td width='15%'>" + eventData["Engine_size"] + "</td><td width='10%'>" + eventData["userID"] + "</td><td width='10%'>" + eventData["firstName"] + "</td><td width='10%'>" + eventData["lastName"] + "</td></tr>";
      } else {
  ...
  update(eventData) {
    if(eventData.$class === this.eventClass) {
      var rowData = "";
      if(this.user === "car") {
        rowData = "<tr class='anim highlight'><td width='10%'>" + eventData["carid"] + "</td><td width='10%'>" + eventData["brand"] + "</td><td width='10%'>" + eventData["model"] + "</td><td width='15%'>" + eventData["Engine_size"] + "</td><td width='10%'>" + eventData["userID"] + "</td><td width='10%'>" + eventData["firstName"] + "</td><td width='10%'>" + eventData["lastName"] + "</td></tr>";
      } else {
